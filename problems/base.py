...
class TextEmbeddingDataset(Dataset):
    def __init__(self, dataset, dataset_name):
        ...
        EMBEDDING_FILE = self.dataset.data_path + "dataset_embeddings.pt"
        if os.path.exists(EMBEDDING_FILE):
            PRECOMPUTED_EMBEDDINGS = torch.load(EMBEDDING_FILE)
        else:
            PRECOMPUTED_EMBEDDINGS = {}
            print(f"Warning: {EMBEDDING_FILE} not found. Text embeddings will be missing.")
        # 根据数据集名字找对应的 Embedding
        # 如果找不到精确匹配，可以尝试模糊匹配 (比如 "fluids.incompressible.Sines" 没找到就找 "fluids.incompressible")
        if dataset_name in PRECOMPUTED_EMBEDDINGS:
            self.text_embed = PRECOMPUTED_EMBEDDINGS[dataset_name]
        else:
            # 简单的回退逻辑，或者报错
            # 这里为了演示，假设必须找到
             # 尝试找通用的 key
            found = False
            for k in PRECOMPUTED_EMBEDDINGS:
                if k in dataset_name:
                    self.text_embed = PRECOMPUTED_EMBEDDINGS[k]
                    found = True
                    break
            if not found:
                 # 创建一个全零的或者随机的作为占位
                self.text_embed = torch.zeros(768) 
        
        # 代理属性
        self.resolution = getattr(dataset, "resolution", 128)
        self.input_dim = getattr(dataset, "input_dim", 1)
        self.output_dim = getattr(dataset, "output_dim", 1)
        self.channel_slice_list = getattr(dataset, "channel_slice_list", [])
        self.printable_channel_description = getattr(dataset, "printable_channel_description", [])
    ...
    def __getitem__(self, idx):
        ...
        data["text_embedding"] = self.text_embed 
        return data
...
def get_dataset(dataset, **kwargs):
    ...
    **kwargs overwrite the default settings.
    ...
    if ".time" in dataset:
        return TextEmbeddingDataset(TimeWrapper(dset(**kwargs)), dataset)
    else:
        return TextEmbeddingDataset(dset(**kwargs), dataset)
